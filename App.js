  ...
  return (
    <View style={[backgroundStyle, styles.outer]}>
      ...
      <ImageBackground
        blurRadius={10}
        source={{
          uri: 'https://www.mriglobal.org/wp-content/uploads/2019/09/COVID-19-Virus-Background-Emerging-Infectious-Disease.jpg',
        }}
        style={{ height: height, width: width }}
      />
      <Text style={styles.title}>
        {'Détection de Maladies sur les patients'}
      </Text>

      {image && (
        <TouchableOpacity onPress={clearOutput} style={styles.clearStyle}>
          <Icon type={'fontAwesome'} name={'trash'} size={25} color={'#FFF'} />
        </TouchableOpacity>
      )}
      {(image?.length && (
      ...
      {(result && label && (
        ...
        (image && <Text style={styles.emptyText}>{label}</Text>) || (
          <Text style={styles.emptyText}>
            Choisir une photo d'un patient soit par Camera ou Gallery 😎
          </Text>
        )}
      <View style={styles.btn}>
        ...
        <TouchableOpacity
          ...
          style={styles.btnStyle}>
          <Icon type={'fontAwesome'} name={'photo'} size={30} />
        </TouchableOpacity>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  container: {
    ...
    padding: 10, // Optional: Add padding to adjust the space around the text
  },
  title: {
    position: 'absolute',
    top: (isIOS && 35) || 10,
  ...
  imageIcon: { height: 40, width: 40, tintColor: 'red' },
  emptyText: {
    position: 'absolute',
    top: height / 1.6,
    color: '#FFF',
